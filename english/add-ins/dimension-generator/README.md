...
| `DIMFAST` | `BP_DIM_FAST` | Dimension the selected objects with the last-used settings |

The same actions are available on the **BIMIL** ribbon tab as the **Dimension Generator** and **Apply Last** buttons.
...
* [**Apply Last**](apply-last.md)**:**\
  Reapply your most recent settings to a new selection straight from the ribbon, without opening the window.
* **Two ways to pick targets:** *Select objects* to dimension only the objects you pick, or *Select layer* to dimension every line and polyline on a chosen layer.
...
* **Preview help:** The **?** preview shows exactly how the current mode and option combination will look before you Apply.
...
* Your last inputs are restored each time you reopen the window during the AutoCAD session.
